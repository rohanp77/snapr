...
TVec<TInt> TLSHash::ComputeSignature(TFltV Datum) {
  TVec<TInt> SigV;
...
TLSHash::TLSHash(int Bands, int Rows, int Dim, int ExpectedSz,
  DistMeasure Type, int NearDist) : Bands(Bands), 
...
void TLSHash::Init() {
  ...
  for (int i=0; i<Bands; i++) {
    SigBucketVHV.Add(THashMP<TInt, TIntV> (ExpectedSz*2));
  }
}
...
void TLSHash::Insert(TFltV Datum) {
  ...
  TVec<TInt> SigV = ComputeSignature(Datum);
  //#pragma omp parallel for
  for (int i=0; i<Bands; i++) {
    TInt& Sig = SigV[i];
    THashMP<TInt, TIntV> &SigBucketVH = SigBucketVHV[i];

    int KeyId = SigBucketVH.GetKeyId(Sig);
    if (Id > 196600)
      printf("%d\n", KeyId);
    if (KeyId == -1) {
      KeyId = SigBucketVH.AddKey(Sig);
      if (Id > 196600)
        printf("%d\n", KeyId);
    }
