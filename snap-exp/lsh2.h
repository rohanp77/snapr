...
private:
  ...
  TVec<TFltV> DataV;
  TVec<THashMP<TInt, TIntV> > SigBucketVHV;

  TVec<TInt> ComputeSignature(TFltV Datum);
  bool IsNear(TFltV Datum1, TFltV Datum2);

public:
  TLSHash();
  TLSHash(int Bands, int Rows, int Dim, int ExpectedSz,
    DistMeasure Type, int NearDist = 1);
  ~TLSHash();
